...
public class NodoJarras {
    ...
    public void aplicarReglas(ListaEnlazada<NodoJarras> reglas) {
        sucesores = new ListaEnlazada<>();
        if (reglas.getSize() > 0) {
            NodoJarras[] reglasArreglo = reglas.toArray();
            for (int i = 0; i < reglasArreglo.length; i++) {
                NodoJarras aux = reglasArreglo[i];
                aux.setPadre(this); //this es el objeto actual. NodoJarras que es el sujeto PAdre
